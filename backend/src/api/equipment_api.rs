...
pub async fn get_equipment_summaries(db: Data<DbRef>) -> impl Responder {
    let coll = db.equipment_coll.clone();
    let summaries = generic_get_summaries(coll).await;
    actix_web::HttpResponse::Ok().json(summaries)
}

#[get("/api/equipment/{slug}")]
pub async fn get_equipment(db: Data<DbRef>, slug: Path<String>) -> impl Responder {
    let coll = db.equipment_coll.clone();
...
pub async fn get_equipment_preview(db: Data<DbRef>, slug: Path<String>) -> impl Responder {
    let coll = db.equipment_coll.clone();
    match generic_get_single(coll, slug.to_string()).await {
        Some(data) => {
