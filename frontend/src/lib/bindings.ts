...
export type Condition = { _id: string; fvttId: string; name: string; group: string | null; description: string; duration: ConditionDuration; overrides?: string[]; publication: Publication; removable: boolean | null; rules: Rule[]; slug: string; value: number | null; type: string }

export type ConditionDuration = { expiry: boolean | null; perpetual: boolean | null; text: string | null; unit: string; value: number }

export type Damage = { base: DamageRoll }

export type DamageCategory = "splash" | "persistent"

export type DamageKind = "damage" | "healing"

export type DamageRoll = { applyMod: boolean | null; category: DamageCategory | null; damageType: Either<DamageType, string> | null; dice: number | null; die: Either<Die, number> | null; formula: string | null; kind: DamageKind | null; kinds?: DamageKind[]; materials?: string[]; type: string | null; persistent: PersistentDamage | null; value: JsonValue | null }

export type DamageType = "bludgeoning" | "piercing" | "slashing" | "mental" | "force" | "poison" | "fire" | "acid" | "spirit" | "sonic" | "electricity" | "cold"

export type Die = "D0" | "D4" | "D6" | "D8" | "D10" | "D12" | "D20" | "D100"
...
export type Equipment = { _id: string; fvttId: string; name: string; acBonus: number | null; apex: Attribute | null; baseItem: string | null; bonus: number | null; bulk: EquipmentBulk | null; category: string | null; checkPenalty: number | null; damage: DamageRoll | null; description: string; dexCap: number | null; carryType: string | null; group: string | null; level: number | null; price: EquipmentPrice | null; quantity: number | null; range: number | null; runes: EquipmentRunes | null; size: string | null; speedPenalty: number | null; splashDamage: number | null; stackGroup: string | null; stowing: boolean | null; strength: number | null; usage: EquipmentUsage | null; uses: EquipmentUses | null; rules: Rule[]; publication: Publication; rarity: string; traits?: string[]; slug: string; type: string }

export type EquipmentBulk = { heldOrStowed: Either<boolean, number> | null; capacity: number | null; ignored: number | null; value: number }

export type EquipmentPrice = { per: number | null; cp: number | null; sp: number | null; gp: number | null; pp: number | null }

export type EquipmentRunes = { potency: number | null; property?: string[]; reinforcing: number | null; resilient: number | null; striking: number | null }

export type EquipmentUsage = { canBeAmmo: boolean | null; value: string }

export type EquipmentUses = { autoDestroy: boolean | null; max: number; value: number }
...
export type GrantItemRule = { track: boolean | null; inMemoryOnly: boolean | null; reevaluateOnUpdate: boolean | null; allowDuplicate: boolean | null; nestFeat: boolean | null; preselectCoices: JsonValue | null; flag: string | null; uuid: string; predicate?: MVecType<RulePredicateFilter>; preselectChoices?: { [key in string]: string }; onDeleteActions: JsonValue | null; alterations: JsonValue | null; priority: number | null }
...
export type PersistentDamage = { faces: number | null; number: number; type: PersistentDamageType | null }

export type PersistentDamageType = "bleed" | "poison" | "fire" | "acid" | "piercing" | "void"
...
export type SavingThrow = "fortitude" | "reflex" | "will" | "ac" | "fortitude-dc" | "reflex-dc" | "will-dc"
...
export type Spell = { _id: string; fvttId: string; name: string; area: SpellArea | null; cost: string | null; counteraction: boolean; damage?: DamageRoll[]; defense: SpellDefense | null; description: string; heightening: SpellHeightening | null; level: number; publication: Publication; range: string | null; requirements: string | null; target: string | null; time: string | null; rules?: Rule[]; traditions?: string[]; rarity: string; traits?: string[]; slug: string; type: string }

export type SpellArea = { areaType: SpellAreaType | null; type: SpellAreaType; value: number; details: string | null }

export type SpellAreaType = "burst" | "cone" | "line" | "emanation" | "square" | "cylinder" | "cube"

export type SpellDefense = { save: SpellDefenseSave | null; passive: SpellDefenseSavePassive | null }

export type SpellDefenseSave = { basic: boolean; statistic: SavingThrow }

export type SpellDefenseSavePassive = { statistic: SavingThrow }

export type SpellHeightening = { damage: { [key in string]: string } | null; levels: { [key in string]: SpellHeighteningLevel } | null; interval: number | null; type: SpellHeighteningType | null }

export type SpellHeighteningLevel = { damage: { [key in string]: DamageRoll } | null; area: SpellArea | null; range: string | null; target: string | null; time: string | null; traits?: string[] }

export type SpellHeighteningType = "interval" | "fixed"
