...
export function transformLinks(text: string): string {
    ...
    for (const match of matches) {
        ...
        const uuid = match[1].replaceAll("Compendium.pf2e.", "").replaceAll(/\..+?\./g, ".").replaceAll(".", "/");
        ...
        const compendiumSectionMatched = COMPENDIUM_SECTION_MATCHER.get(uuidSection);
    ...
    while (text.match(EMPTY_HTML_ELEMENT) !== null) {
        ...
        for (const match of matches) {
            ...
            if (firstGroup !== secondGroup) {
                console.error(text)
                throw new Error(`empty html element found, but types didnt match: ${firstGroup}, ${secondGroup}`)
            }
...
export function abbreviateSource(source: string): string {
    ...
    return COMPENDIUM_SOURCE_ABBREVIATIONS.get(sourceTrimmed.replace('Pathfinder ', '')) ?? sourceTrimmed
}
